...
def get_x_coordinate(width, year_index):
    ...
    """
    x_coordinate = GRAPH_MARGIN_SIZE + int(((width - GRAPH_MARGIN_SIZE*2) / len(YEARS)) * int(year_index))
    return x_coordinate
...
def draw_fixed_lines(canvas):
    ...
    for i in range(len(YEARS)):
        ...
        canvas.create_line(get_x_coordinate(CANVAS_WIDTH, i),
                           0, get_x_coordinate(CANVAS_WIDTH, i),
...
def draw_names(canvas, name_data, lookup_names):
    """
    Given a dict of baby name data and a list of names, plot
    ...
    def get_y_coordinate(height, rank):
        y_coordinate = GRAPH_MARGIN_SIZE + rank * ((height-2*GRAPH_MARGIN_SIZE)/1000)
        return y_coordinate
    ...
    for name in lookup_names:
        ...
        for x, y in data.items():
            year_rank[int(x)] = int(y)
        for year in YEARS:
            rank = year_rank.get(year, 1000)
            x2, y2 = get_x_coordinate(CANVAS_WIDTH, YEARS.index(year)), get_y_coordinate(CANVAS_HEIGHT, rank)
            if rank >= 1000:
                rank_text = '*'
            else:
                rank_text = str(rank)
            if year > 1900:
                ...
                canvas.create_text(x2 + TEXT_DX, y2, text=f'{name}, {rank_text}', anchor=tkinter.SW, fill=COLORS[color_index])
            else:
                canvas.create_text(x2 + TEXT_DX, y2, text=f'{name}, {rank_text}', anchor=tkinter.SW, fill=COLORS[color_index])
            x1, y1 = x2, y2
